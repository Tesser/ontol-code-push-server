  ...
  describe("Storage management", () => {
    ...
    if (StorageType === AzureStorage) {
      it("should allow reconfiguring of Azure storage credentials", () => {
        ...
        return azureStorage
          .initialize("wrongaccount", "wrongkey")
          .then(
            failOnCallSucceeded,
            /*returnedUnhealthy*/ () => {
              if (!process.env.EMULATED && process.env.AZURE_STORAGE_ACCOUNT && process.env.AZURE_STORAGE_ACCESS_KEY) {
                return azureStorage.initialize(process.env.AZURE_STORAGE_ACCOUNT, process.env.AZURE_STORAGE_ACCESS_KEY);
              } else {
                return azureStorage.initialize();
              }
            }
          )
          .then(() => {
